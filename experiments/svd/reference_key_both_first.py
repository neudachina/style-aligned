...
def svd_exp(prompt, style_name, objects, image_name='output', seed=0):
    # обнуляю первые компоненты у референса для key на всех шагах
    # генерирую сначала с обычным шерингом, потом с обнулением, в конце без шеринга для сравнения
    
    base_path = './images/more-reference/all-key-first-coef'
    ...
    sa_args.svd_reference_key_end = 1
    
    # хочу по горизонтали менять коэффициент обнуления
    # а по вертикали просто разные промпты
    xs = np.linspace(0.5, 0, 6)
        
    for i in xs:
        sa_args.svd_reference_key_coef = i
